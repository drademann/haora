...
func TestNewDay(t *testing.T) {
	t.Run("should initialise the days date", func(t *testing.T) {
		date := app.MockDate(2024, time.February, 21, 14, 58)
		day := app.NewDay(date)
...
func TestSameDay(t *testing.T) {
	testCases := []struct {
		...
		{"dates at exact same time should return true",
			app.MockDate(2024, time.February, 21, 10, 0),
			app.MockDate(2024, time.February, 21, 10, 0),
			true},
		{"dates at same day should return true",
			app.MockDate(2024, time.February, 21, 10, 0),
			app.MockDate(2024, time.February, 21, 15, 22),
			true},
		{"dates at different days should return false",
			app.MockDate(2024, time.February, 21, 10, 0),
			app.MockDate(2024, time.February, 12, 10, 0),
			false},
		{"dates at different month should return false",
			app.MockDate(2024, time.February, 21, 10, 0),
			app.MockDate(2024, time.December, 21, 10, 0),
			false},
		{"dates at different years should return false",
			app.MockDate(2024, time.February, 21, 10, 0),
			app.MockDate(2025, time.February, 21, 10, 0),
			false},
	}
